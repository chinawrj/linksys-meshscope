...
MODULE_PATH = REPO_ROOT / "tools" / "linksys_mx_repack.py"
SPEC = importlib.util.spec_from_file_location("linksys_mx_repack", MODULE_PATH)
repack = importlib.util.module_from_spec(SPEC)
assert SPEC.loader is not None
SPEC.loader.exec_module(repack)


class LinksysMXRepackTests(unittest.TestCase):
    def test_posix_cksum_matches_reference_vector(self):
        self.assertEqual(930766865, repack.posix_cksum(b"123456789"))

    def test_assemble_preserves_prefix_and_updates_footer_for_both_skus(self):
        for sku in sorted(repack.SUPPORTED_SKUS):
            with self.subTest(sku=sku):
                ...
                footer[32:40] = (
                    f"{repack.posix_cksum(old_body):08X}".encode("ascii")
                )
                ...
                self.assertEqual(sku, manifest["sku"])
                self.assertEqual(len(result), manifest["output_size"])
                self.assertEqual(
                    (
                        f"{repack.posix_cksum(result[:-repack.FOOTER_SIZE]):08X}"
                    ).encode("ascii"),
                    result[
                        -repack.FOOTER_SIZE + 32 : -repack.FOOTER_SIZE + 40
                    ],
                )

    def test_assemble_rejects_wrong_expected_sku(self):
        prefix = bytearray(repack.UBI_OFFSET)
        old_ubi = bytearray(3 * repack.PEB_SIZE)
        new_ubi = bytearray(3 * repack.PEB_SIZE)
        for image in (old_ubi, new_ubi):
            for offset in range(0, len(image), repack.PEB_SIZE):
                image[offset : offset + 4] = b"UBI#"
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with self.assertRaisesRegex(ValueError, "footer SKU mismatch"):
                repack.assemble_image(
                    original, replacement, output, "MX5300"
                )
